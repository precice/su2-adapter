...
void CIteration::SetGrid_Movement(CGeometry*** geometry_container, CSurfaceMovement** surface_movement,
                                  ...
                                  CSolver**** solver_container, CConfig** config_container, unsigned short val_iZone,
                                  unsigned long IntIter, unsigned long ExtIter) {
  unsigned short iDim;
  ...
  switch (Kind_Grid_Movement) {
    case RIGID_MOTION:
      ...
      grid_movement[val_iZone]->Rigid_Translation(geometry_container[val_iZone][MESH_0], config_container[val_iZone],
                                                  val_iZone, ExtIter);
      grid_movement[val_iZone]->Rigid_Plunging(geometry_container[val_iZone][MESH_0], config_container[val_iZone],
                                               val_iZone, ExtIter);
      grid_movement[val_iZone]->Rigid_Pitching(geometry_container[val_iZone][MESH_0], config_container[val_iZone],
                                               val_iZone, ExtIter);
      grid_movement[val_iZone]->Rigid_Rotation(geometry_container[val_iZone][MESH_0], config_container[val_iZone],
                                               val_iZone, ExtIter);
    ...
    case DEFORMING:
      ...
      if (rank == MASTER_NODE) cout << endl << " Updating surface positions." << endl;
      ...
      surface_movement[val_iZone]->Surface_Plunging(geometry_container[val_iZone][MESH_0], config_container[val_iZone],
                                                    ExtIter, val_iZone);
      /*--- Deform the volume grid around the new boundary locations ---*/
      ...
      surface_movement[val_iZone]->Surface_Pitching(geometry_container[val_iZone][MESH_0], config_container[val_iZone],
                                                    ExtIter, val_iZone);
      /*--- Deform the volume grid around the new boundary locations ---*/
      ...
      surface_movement[val_iZone]->Surface_Rotating(geometry_container[val_iZone][MESH_0], config_container[val_iZone],
                                                    ExtIter, val_iZone);
      /*--- Deform the volume grid around the new boundary locations ---*/
    ...
    case AEROELASTIC_RIGID_MOTION:
      ...
      /*--- Use the if statement to move the grid only at selected dual time step iterations. ---*/
        ...
        solver_container[val_iZone][MESH_0][FLOW_SOL]->Aeroelastic(
            surface_movement[val_iZone], geometry_container[val_iZone][MESH_0], config_container[val_iZone], ExtIter);
    ...
    case ELASTICITY:
      ...
      if (ExtIter != 0) {
        if (rank == MASTER_NODE) cout << " Deforming the grid using the Linear Elasticity solution." << endl;
    ...
    case FLUID_STRUCTURE:
      ...
      if (!adjoint && !stat_mesh) {
        ...
        if (rank == MASTER_NODE) cout << "The mesh is up-to-date. Using previously stored grid velocities." << endl;
      }
    ...
    case FLUID_STRUCTURE_STATIC:
      ...
      if ((rank == MASTER_NODE) && (!discrete_adjoint)) cout << "Deforming the volume grid." << endl;
      ...
      if ((rank == MASTER_NODE) && (!discrete_adjoint)) cout << "There is no grid velocity." << endl;
    ...
    default:
      ...
      if (rank == MASTER_NODE) cout << "No mesh motion specified." << endl;

      break;
  }
}

void CIteration::Preprocess(COutput* output, CIntegration*** integration_container, CGeometry*** geometry_container,
...
void CIteration::Iterate(COutput* output, CIntegration*** integration_container, CGeometry*** geometry_container,
                         ...
                         CSurfaceMovement** surface_movement, CVolumetricMovement** grid_movement,
                         CFreeFormDefBox*** FFDBox, unsigned short val_iZone) {}
void CIteration::Update(COutput* output, CIntegration*** integration_container, CGeometry*** geometry_container,
                        CSolver**** solver_container, CNumerics***** numerics_container, CConfig** config_container,
                        CSurfaceMovement** surface_movement, CVolumetricMovement** grid_movement,
                        CFreeFormDefBox*** FFDBox, unsigned short val_iZone) {}
void CIteration::Monitor() {}
void CIteration::Output() {}
void CIteration::Postprocess(COutput* output, CIntegration*** integration_container, CGeometry*** geometry_container,
                             CSolver**** solver_container, CNumerics***** numerics_container,
                             CConfig** config_container, CSurfaceMovement** surface_movement,
...
void CFluidIteration::Preprocess(COutput* output, CIntegration*** integration_container,
  ...
  if (fsi && (FSIIter == 0)) {
    solver_container[val_iZone][MESH_0][FLOW_SOL]->SetInitialCondition(
        geometry_container[val_iZone], solver_container[val_iZone], config_container[val_iZone], ExtIter);
  }
...
void CFluidIteration::Iterate(COutput* output, CIntegration*** integration_container, CGeometry*** geometry_container,
                              CSolver**** solver_container, CNumerics***** numerics_container,
                              CConfig** config_container, CSurfaceMovement** surface_movement,
                              CVolumetricMovement** grid_movement, CFreeFormDefBox*** FFDBox,
  ...
  bool unsteady = (config_container[val_iZone]->GetUnsteady_Simulation() == DT_STEPPING_1ST) ||
                  (config_container[val_iZone]->GetUnsteady_Simulation() == DT_STEPPING_2ND);
  bool frozen_visc =
      (config_container[val_iZone]->GetContinuous_Adjoint() && config_container[val_iZone]->GetFrozen_Visc_Cont()) ||
  ...
  integration_container[val_iZone][FLOW_SOL]->MultiGrid_Iteration(
      geometry_container, solver_container, numerics_container, config_container, RUNTIME_FLOW_SYS, IntIter, val_iZone);
  ...
  if ((config_container[val_iZone]->GetKind_Solver() == RANS) ||
    ...
    integration_container[val_iZone][TURB_SOL]->SingleGrid_Iteration(geometry_container, solver_container,
                                                                     numerics_container, config_container,
                                                                     RUNTIME_TURB_SYS, IntIter, val_iZone);
    ...
    if (config_container[val_iZone]->GetKind_Trans_Model() == LM) {
      ...
      integration_container[val_iZone][TRANS_SOL]->SingleGrid_Iteration(geometry_container, solver_container,
                                                                        numerics_container, config_container,
                                                                        RUNTIME_TRANS_SYS, IntIter, val_iZone);
    }
  }
  ...
  if ((config_container[val_iZone]->GetGrid_Movement()) && (config_container[val_iZone]->GetAeroelastic_Simulation()) &&
      unsteady) {
    SetGrid_Movement(geometry_container, surface_movement, grid_movement, FFDBox, solver_container, config_container,
                     val_iZone, IntIter, ExtIter);
...
void CFluidIteration::Update(COutput* output, CIntegration*** integration_container, CGeometry*** geometry_container,
                             CSolver**** solver_container, CNumerics***** numerics_container,
                             CConfig** config_container, CSurfaceMovement** surface_movement,
                             CVolumetricMovement** grid_movement, CFreeFormDefBox*** FFDBox, unsigned short val_iZone) {
  ...
  if ((config_container[val_iZone]->GetUnsteady_Simulation() == DT_STEPPING_1ST) ||
    ...
    for (iMesh = 0; iMesh <= config_container[val_iZone]->GetnMGLevels(); iMesh++) {
      integration_container[val_iZone][FLOW_SOL]->SetDualTime_Solver(geometry_container[val_iZone][iMesh],
                                                                     solver_container[val_iZone][iMesh][FLOW_SOL],
                                                                     config_container[val_iZone], iMesh);
    ...
    if ((config_container[val_iZone]->GetKind_Solver() == RANS) ||
      ...
      integration_container[val_iZone][TURB_SOL]->SetDualTime_Solver(geometry_container[val_iZone][MESH_0],
                                                                     solver_container[val_iZone][MESH_0][TURB_SOL],
    ...
    if (config_container[val_iZone]->GetKind_Trans_Model() == LM) {
      integration_container[val_iZone][TRANS_SOL]->SetDualTime_Solver(geometry_container[val_iZone][MESH_0],
                                                                      solver_container[val_iZone][MESH_0][TRANS_SOL],
                                                                      config_container[val_iZone], MESH_0);
...
void CFluidIteration::Postprocess(COutput* output, CIntegration*** integration_container,
                                  CGeometry*** geometry_container, CSolver**** solver_container,
                                  CNumerics***** numerics_container, CConfig** config_container,
                                  CSurfaceMovement** surface_movement, CVolumetricMovement** grid_movement,
                                  CFreeFormDefBox*** FFDBox, unsigned short val_iZone) {}

void CFluidIteration::SetWind_GustField(CConfig* config_container, CGeometry** geometry_container,
  ...
  su2double Uinf =
      solver_container[MESH_0][FLOW_SOL]->GetVelocity_Inf(0);  // Assumption gust moves at infinity velocity
  ...
  for (iMGlevel = 0; iMGlevel <= nMGlevel; iMGlevel++) {
    ...
    for (iPoint = 0; iPoint < geometry_container[iMGlevel]->GetnPoint(); iPoint++) {
      ...
      if (Kind_Grid_Movement == GUST) {
        for (iDim = 0; iDim < nDim; iDim++) geometry_container[iMGlevel]->node[iPoint]->SetGridVel(iDim, 0.0);
      }
...
void CTurboIteration::Preprocess(COutput* output, CIntegration*** integration_container,
                                 CGeometry*** geometry_container, CSolver**** solver_container,
                                 CNumerics***** numerics_container, CConfig** config_container,
                                 CSurfaceMovement** surface_movement, CVolumetricMovement** grid_movement,
                                 CFreeFormDefBox*** FFDBox, unsigned short val_iZone) {
  /*--- Average quantities at the inflow and outflow boundaries ---*/
  solver_container[val_iZone][MESH_0][FLOW_SOL]->TurboAverageProcess(
      solver_container[val_iZone][MESH_0], geometry_container[val_iZone][MESH_0], config_container[val_iZone], INFLOW);
  solver_container[val_iZone][MESH_0][FLOW_SOL]->TurboAverageProcess(
      solver_container[val_iZone][MESH_0], geometry_container[val_iZone][MESH_0], config_container[val_iZone], OUTFLOW);
}

void CTurboIteration::Postprocess(COutput* output, CIntegration*** integration_container,
                                  CGeometry*** geometry_container, CSolver**** solver_container,
                                  CNumerics***** numerics_container, CConfig** config_container,
                                  CSurfaceMovement** surface_movement, CVolumetricMovement** grid_movement,
                                  CFreeFormDefBox*** FFDBox, unsigned short val_iZone) {
  ...
  solver_container[val_iZone][MESH_0][FLOW_SOL]->GatherInOutAverageValues(config_container[val_iZone],
                                                                          geometry_container[val_iZone][MESH_0]);
}
...
void CWaveIteration::Preprocess(COutput* output, CIntegration*** integration_container, CGeometry*** geometry_container,
                                CSolver**** solver_container, CNumerics***** numerics_container,
                                CConfig** config_container, CSurfaceMovement** surface_movement,
                                CVolumetricMovement** grid_movement, CFreeFormDefBox*** FFDBox,
                                unsigned short val_iZone) {}
...
void CWaveIteration::Iterate(COutput* output, CIntegration*** integration_container, CGeometry*** geometry_container,
  ...
  integration_container[val_iZone][WAVE_SOL]->SingleGrid_Iteration(
      geometry_container, solver_container, numerics_container, config_container, RUNTIME_WAVE_SYS, IntIter, val_iZone);
  ...
  if ((config_container[val_iZone]->GetUnsteady_Simulation() == DT_STEPPING_1ST) ||
    ...
    for (IntIter = 1; IntIter < config_container[val_iZone]->GetUnst_nIntIter(); IntIter++) {
      output->SetConvHistory_Body(NULL, geometry_container, solver_container, config_container, integration_container,
                                  true, 0.0, val_iZone);
      config_container[val_iZone]->SetIntIter(IntIter);
      integration_container[val_iZone][WAVE_SOL]->SingleGrid_Iteration(geometry_container, solver_container,
                                                                       numerics_container, config_container,
                                                                       RUNTIME_WAVE_SYS, IntIter, val_iZone);
      if (integration_container[val_iZone][WAVE_SOL]->GetConvergence()) break;
    }
  }
}

void CWaveIteration::Update(COutput* output, CIntegration*** integration_container, CGeometry*** geometry_container,
                            CSolver**** solver_container, CNumerics***** numerics_container, CConfig** config_container,
                            CSurfaceMovement** surface_movement, CVolumetricMovement** grid_movement,
                            CFreeFormDefBox*** FFDBox, unsigned short val_iZone) {
  unsigned short iMesh;
  ...
  if ((config_container[val_iZone]->GetUnsteady_Simulation() == DT_STEPPING_1ST) ||
    ...
    for (iMesh = 0; iMesh <= config_container[val_iZone]->GetnMGLevels(); iMesh++) {
      integration_container[val_iZone][WAVE_SOL]->SetDualTime_Solver(geometry_container[val_iZone][iMesh],
                                                                     solver_container[val_iZone][iMesh][WAVE_SOL],
                                                                     config_container[val_iZone], iMesh);
    ...
    Physical_t = (ExtIter + 1) * Physical_dt;
    if (Physical_t >= config_container[val_iZone]->GetTotal_UnstTime())
      integration_container[val_iZone][WAVE_SOL]->SetConvergence(true);
  }
}
...
void CWaveIteration::Postprocess(COutput* output, CIntegration*** integration_container,
                                 CGeometry*** geometry_container, CSolver**** solver_container,
                                 CNumerics***** numerics_container, CConfig** config_container,
                                 CSurfaceMovement** surface_movement, CVolumetricMovement** grid_movement,
                                 CFreeFormDefBox*** FFDBox, unsigned short val_iZone) {}
...
void CHeatIteration::Preprocess(COutput* output, CIntegration*** integration_container, CGeometry*** geometry_container,
                                CSolver**** solver_container, CNumerics***** numerics_container,
                                CConfig** config_container, CSurfaceMovement** surface_movement,
                                CVolumetricMovement** grid_movement, CFreeFormDefBox*** FFDBox,
                                unsigned short val_iZone) {}
...
void CHeatIteration::Iterate(COutput* output, CIntegration*** integration_container, CGeometry*** geometry_container,
  ...
  integration_container[val_iZone][HEAT_SOL]->SingleGrid_Iteration(
      geometry_container, solver_container, numerics_container, config_container, RUNTIME_HEAT_SYS, IntIter, val_iZone);
  ...
  if ((config_container[val_iZone]->GetUnsteady_Simulation() == DT_STEPPING_1ST) ||
    ...
    for (IntIter = 1; IntIter < config_container[val_iZone]->GetUnst_nIntIter(); IntIter++) {
      output->SetConvHistory_Body(NULL, geometry_container, solver_container, config_container, integration_container,
                                  true, 0.0, val_iZone);
      config_container[val_iZone]->SetIntIter(IntIter);
      integration_container[val_iZone][HEAT_SOL]->SingleGrid_Iteration(geometry_container, solver_container,
                                                                       numerics_container, config_container,
                                                                       RUNTIME_HEAT_SYS, IntIter, val_iZone);
      if (integration_container[val_iZone][HEAT_SOL]->GetConvergence()) break;
    }
  }
}

void CHeatIteration::Update(COutput* output, CIntegration*** integration_container, CGeometry*** geometry_container,
                            CSolver**** solver_container, CNumerics***** numerics_container, CConfig** config_container,
                            CSurfaceMovement** surface_movement, CVolumetricMovement** grid_movement,
                            CFreeFormDefBox*** FFDBox, unsigned short val_iZone) {
  unsigned short iMesh;
  ...
  if ((config_container[val_iZone]->GetUnsteady_Simulation() == DT_STEPPING_1ST) ||
    ...
    /*--- Update dual time solver ---*/
    for (iMesh = 0; iMesh <= config_container[val_iZone]->GetnMGLevels(); iMesh++) {
      integration_container[val_iZone][HEAT_SOL]->SetDualTime_Solver(geometry_container[val_iZone][iMesh],
                                                                     solver_container[val_iZone][iMesh][HEAT_SOL],
                                                                     config_container[val_iZone], iMesh);
...
void CHeatIteration::Postprocess(COutput* output, CIntegration*** integration_container,
                                 CGeometry*** geometry_container, CSolver**** solver_container,
                                 CNumerics***** numerics_container, CConfig** config_container,
                                 CSurfaceMovement** surface_movement, CVolumetricMovement** grid_movement,
                                 CFreeFormDefBox*** FFDBox, unsigned short val_iZone) {}
...
void CPoissonIteration::Preprocess(COutput* output, CIntegration*** integration_container,
                                   CGeometry*** geometry_container, CSolver**** solver_container,
                                   CNumerics***** numerics_container, CConfig** config_container,
                                   CSurfaceMovement** surface_movement, CVolumetricMovement** grid_movement,
                                   CFreeFormDefBox*** FFDBox, unsigned short val_iZone) {}
void CPoissonIteration::Iterate(COutput* output, CIntegration*** integration_container, CGeometry*** geometry_container,
  ...
  integration_container[val_iZone][POISSON_SOL]->SingleGrid_Iteration(geometry_container, solver_container,
                                                                      numerics_container, config_container,
                                                                      RUNTIME_POISSON_SYS, IntIter, val_iZone);
}
void CPoissonIteration::Update(COutput* output, CIntegration*** integration_container, CGeometry*** geometry_container,
                               CSolver**** solver_container, CNumerics***** numerics_container,
                               CConfig** config_container, CSurfaceMovement** surface_movement,
                               CVolumetricMovement** grid_movement, CFreeFormDefBox*** FFDBox,
                               unsigned short val_iZone) {}
...
void CPoissonIteration::Postprocess(COutput* output, CIntegration*** integration_container,
                                    CGeometry*** geometry_container, CSolver**** solver_container,
                                    CNumerics***** numerics_container, CConfig** config_container,
                                    CSurfaceMovement** surface_movement, CVolumetricMovement** grid_movement,
                                    CFreeFormDefBox*** FFDBox, unsigned short val_iZone) {}
...
void CFEM_StructuralAnalysis::Iterate(COutput* output, CIntegration*** integration_container,
                                      CGeometry*** geometry_container, CSolver**** solver_container,
                                      CNumerics***** numerics_container, CConfig** config_container,
                                      CSurfaceMovement** surface_movement, CVolumetricMovement** grid_movement,
                                      CFreeFormDefBox*** FFDBox, unsigned short val_iZone) {
  ...
  bool nonlinear = (config_container[val_iZone]->GetGeometricConditions() ==
                    LARGE_DEFORMATIONS);  // Geometrically non-linear problems
  bool linear = (config_container[val_iZone]->GetGeometricConditions() ==
                 SMALL_DEFORMATIONS);  // Geometrically non-linear problems
  ...
  bool initial_calc_restart = (SU2_TYPE::Int(config_container[val_iZone]->GetExtIter()) ==
                               config_container[val_iZone]->GetDyn_RestartIter());  // Initial calculation for restart
  ...
  if (linear) {
    ...
    integration_container[val_iZone][FEA_SOL]->Structural_Iteration(geometry_container, solver_container,
                                                                    numerics_container, config_container,
                                                                    RUNTIME_FEA_SYS, IntIter, val_iZone);

  }
  /*--- If the structure is held static and the solver is nonlinear, we don't need to solve for static time, but we need
     to compute Mass Matrix and Integration constants ---*/
    ...
    /*--- The incremental load is only used in nonlinear cases ---*/
      ...
      solver_container[val_iZone][MESH_0][FEA_SOL]->SetInitialCondition(
          geometry_container[val_iZone], solver_container[val_iZone], config_container[val_iZone], ExtIter);
      ...
      meetCriteria =
          ((Residual_UTOL < Criteria_UTOL) && (Residual_RTOL < Criteria_RTOL) && (Residual_ETOL < Criteria_ETOL));
      ...
      if (meetCriteria) {
        for (IntIter = 2; IntIter < config_container[val_iZone]->GetDyn_nIntIter(); IntIter++) {
          ...
          output->SetConvHistory_Body(NULL, geometry_container, solver_container, config_container,
                                      integration_container, true, 0.0, val_iZone);
      ...
      /*--- If the criteria is not met, a whole set of subiterations for the different loads must be done ---*/
        ...
        solver_container[val_iZone][MESH_0][FEA_SOL]->ResetInitialCondition(
            geometry_container[val_iZone], solver_container[val_iZone], config_container[val_iZone], ExtIter);
        ...
        for (iIncrement = 0; iIncrement < nIncrements; iIncrement++) {
          ...
          /*--- Set the load increment and the initial condition, and output the parameters of UTOL, RTOL, ETOL for the
           * previous iteration ---*/
          ...
          if (rank == MASTER_NODE) {
            cout << endl;
            cout << "-- Incremental load: increment " << iIncrement + 1 << " ------------------------------------------"
                 << endl;
          }
    ...
    solver_container[val_iZone][MESH_0][FEA_SOL]->Preprocessing(
        geometry_container[val_iZone][MESH_0], solver_container[val_iZone][MESH_0], config_container[val_iZone],
        numerics_container[val_iZone][MESH_0][FEA_SOL], MESH_0, 0, RUNTIME_FEA_SYS, false);
  }
  ...
  switch (config_container[val_iZone]->GetKind_ObjFunc()) {
    case REFERENCE_GEOMETRY:
      if ((config_container[val_iZone]->GetDV_FEA() == YOUNG_MODULUS) ||
          (config_container[val_iZone]->GetDV_FEA() == DENSITY_VAL)) {
        solver_container[val_iZone][MESH_0][FEA_SOL]->Stiffness_Penalty(
            geometry_container[val_iZone][MESH_0], solver_container[val_iZone][MESH_0],
            numerics_container[val_iZone][MESH_0][FEA_SOL], config_container[val_iZone]);
      }
      solver_container[val_iZone][MESH_0][FEA_SOL]->Compute_OFRefGeom(
          geometry_container[val_iZone][MESH_0], solver_container[val_iZone][MESH_0], config_container[val_iZone]);
      break;
    case REFERENCE_NODE:
      if ((config_container[val_iZone]->GetDV_FEA() == YOUNG_MODULUS) ||
      ...
      solver_container[val_iZone][MESH_0][FEA_SOL]->Compute_OFRefNode(
          geometry_container[val_iZone][MESH_0], solver_container[val_iZone][MESH_0], config_container[val_iZone]);
      break;
  }
}

void CFEM_StructuralAnalysis::Update(COutput* output, CIntegration*** integration_container,
                                     CGeometry*** geometry_container, CSolver**** solver_container,
                                     CNumerics***** numerics_container, CConfig** config_container,
                                     CSurfaceMovement** surface_movement, CVolumetricMovement** grid_movement,
                                     CFreeFormDefBox*** FFDBox, unsigned short val_iZone) {
  ...
  solver_container[val_iZone][MESH_0][FEA_SOL]->Compute_NodalStress(
      geometry_container[val_iZone][MESH_0], solver_container[val_iZone][MESH_0],
      numerics_container[val_iZone][MESH_0][FEA_SOL], config_container[val_iZone]);
  ...
  if (dynamic) {
    integration_container[val_iZone][FEA_SOL]->SetFEM_StructuralSolver(geometry_container[val_iZone][MESH_0],
    ...
    switch (config_container[val_iZone]->GetKind_TimeIntScheme_FEA()) {
      case (NEWMARK_IMPLICIT):
        solver_container[val_iZone][MESH_0][FEA_SOL]->ImplicitNewmark_Relaxation(
            geometry_container[val_iZone][MESH_0], solver_container[val_iZone][MESH_0], config_container[val_iZone]);
        break;
    }
  }
...
void CFEM_StructuralAnalysis::Postprocess(COutput* output, CIntegration*** integration_container,
                                          CGeometry*** geometry_container, CSolver**** solver_container,
                                          CNumerics***** numerics_container, CConfig** config_container,
                                          CSurfaceMovement** surface_movement, CVolumetricMovement** grid_movement,
                                          CFreeFormDefBox*** FFDBox, unsigned short val_iZone) {}
...
void CAdjFluidIteration::Preprocess(COutput* output, CIntegration*** integration_container,
                                    CGeometry*** geometry_container, CSolver**** solver_container,
                                    CNumerics***** numerics_container, CConfig** config_container,
                                    CSurfaceMovement** surface_movement, CVolumetricMovement** grid_movement,
                                    CFreeFormDefBox*** FFDBox, unsigned short val_iZone) {
  ...
  if (((dynamic_mesh && ExtIter == 0) || config_container[val_iZone]->GetUnsteady_Simulation()) && !harmonic_balance) {
    ...
    solver_container[val_iZone][MESH_0][FLOW_SOL]->LoadRestart(
        geometry_container[val_iZone], solver_container[val_iZone], config_container[val_iZone], Direct_Iter, true);
  }
  ...
  if ((ExtIter == 0) || config_container[val_iZone]->GetUnsteady_Simulation()) {
    ...
    integration_container[val_iZone][FLOW_SOL]->MultiGrid_Iteration(
        geometry_container, solver_container, numerics_container, config_container, RUNTIME_FLOW_SYS, 0, val_iZone);
    ...
    if (config_container[val_iZone]->GetKind_Gradient_Method() == GREEN_GAUSS)
      solver_container[val_iZone][MESH_0][FLOW_SOL]->SetPrimitive_Gradient_GG(geometry_container[val_iZone][MESH_0],
                                                                              config_container[val_iZone]);
    if (config_container[val_iZone]->GetKind_Gradient_Method() == WEIGHTED_LEAST_SQUARES)
      solver_container[val_iZone][MESH_0][FLOW_SOL]->SetPrimitive_Gradient_LS(geometry_container[val_iZone][MESH_0],
                                                                              config_container[val_iZone]);
    ...
    for (iMesh = 0; iMesh <= config_container[val_iZone]->GetnMGLevels(); iMesh++) {
      ...
      solver_container[val_iZone][iMesh][FLOW_SOL]->SetTotal_CD(
          solver_container[val_iZone][MESH_0][FLOW_SOL]->GetTotal_CD());
      solver_container[val_iZone][iMesh][FLOW_SOL]->SetTotal_CL(
          solver_container[val_iZone][MESH_0][FLOW_SOL]->GetTotal_CL());
      solver_container[val_iZone][iMesh][FLOW_SOL]->SetTotal_CT(
          solver_container[val_iZone][MESH_0][FLOW_SOL]->GetTotal_CT());
      solver_container[val_iZone][iMesh][FLOW_SOL]->SetTotal_CQ(
          solver_container[val_iZone][MESH_0][FLOW_SOL]->GetTotal_CQ());

      /*--- Compute the adjoint boundary condition on Euler walls ---*/

      solver_container[val_iZone][iMesh][ADJFLOW_SOL]->SetForceProj_Vector(
          geometry_container[val_iZone][iMesh], solver_container[val_iZone][iMesh], config_container[val_iZone]);
      ...
      if ((config_container[val_iZone]->GetKind_ObjFunc() == EQUIVALENT_AREA) ||
        ...
        solver_container[val_iZone][iMesh][ADJFLOW_SOL]->SetIntBoundary_Jump(
            geometry_container[val_iZone][iMesh], solver_container[val_iZone][iMesh], config_container[val_iZone]);
    }
...
void CAdjFluidIteration::Iterate(COutput* output, CIntegration*** integration_container,
                                 CGeometry*** geometry_container, CSolver**** solver_container,
                                 CNumerics***** numerics_container, CConfig** config_container,
                                 CSurfaceMovement** surface_movement, CVolumetricMovement** grid_movement,
                                 CFreeFormDefBox*** FFDBox, unsigned short val_iZone) {
  ...
  integration_container[val_iZone][ADJFLOW_SOL]->MultiGrid_Iteration(geometry_container, solver_container,
                                                                     numerics_container, config_container,
                                                                     RUNTIME_ADJFLOW_SYS, IntIter, val_iZone);

  /*--- Iteration of the turbulence model adjoint ---*/

  if ((config_container[val_iZone]->GetKind_Solver() == ADJ_RANS) &&
      (!config_container[val_iZone]->GetFrozen_Visc_Cont())) {
    /*--- Adjoint turbulence model solution ---*/

    config_container[val_iZone]->SetGlobalParam(ADJ_RANS, RUNTIME_ADJTURB_SYS, ExtIter);
    integration_container[val_iZone][ADJTURB_SOL]->SingleGrid_Iteration(geometry_container, solver_container,
                                                                        numerics_container, config_container,
                                                                        RUNTIME_ADJTURB_SYS, IntIter, val_iZone);
  }
}
void CAdjFluidIteration::Update(COutput* output, CIntegration*** integration_container, CGeometry*** geometry_container,
                                CSolver**** solver_container, CNumerics***** numerics_container,
                                CConfig** config_container, CSurfaceMovement** surface_movement,
                                CVolumetricMovement** grid_movement, CFreeFormDefBox*** FFDBox,
                                unsigned short val_iZone) {
  ...
  if ((config_container[val_iZone]->GetUnsteady_Simulation() == DT_STEPPING_1ST) ||
    ...
    for (iMesh = 0; iMesh <= config_container[val_iZone]->GetnMGLevels(); iMesh++) {
      integration_container[val_iZone][ADJFLOW_SOL]->SetDualTime_Solver(geometry_container[val_iZone][iMesh],
                                                                        solver_container[val_iZone][iMesh][ADJFLOW_SOL],
                                                                        config_container[val_iZone], iMesh);
    ...
    Physical_t = (ExtIter + 1) * Physical_dt;
    if (Physical_t >= config_container[val_iZone]->GetTotal_UnstTime())
      integration_container[val_iZone][ADJFLOW_SOL]->SetConvergence(true);
  }
}

void CAdjFluidIteration::Monitor() {}
void CAdjFluidIteration::Output() {}
void CAdjFluidIteration::Postprocess(COutput* output, CIntegration*** integration_container,
                                     CGeometry*** geometry_container, CSolver**** solver_container,
                                     CNumerics***** numerics_container, CConfig** config_container,
                                     CSurfaceMovement** surface_movement, CVolumetricMovement** grid_movement,
                                     CFreeFormDefBox*** FFDBox, unsigned short val_iZone) {}
...
void CDiscAdjFluidIteration::Preprocess(COutput* output, CIntegration*** integration_container,
                                        CGeometry*** geometry_container, CSolver**** solver_container,
                                        CNumerics***** numerics_container, CConfig** config_container,
                                        CSurfaceMovement** surface_movement, CVolumetricMovement** grid_movement,
                                        CFreeFormDefBox*** FFDBox, unsigned short val_iZone) {
  ...
  if (config_container[val_iZone]->GetUnsteady_Simulation()) {
    ...
    if ((ExtIter > 0) && dual_time) {
      ...
      for (iMesh = 0; iMesh <= config_container[val_iZone]->GetnMGLevels(); iMesh++) {
        for (iPoint = 0; iPoint < geometry_container[val_iZone][iMesh]->GetnPoint(); iPoint++) {
          solver_container[val_iZone][iMesh][FLOW_SOL]->node[iPoint]->SetSolution(
              solver_container[val_iZone][iMesh][FLOW_SOL]->node[iPoint]->GetSolution_time_n());
          if (turbulent) {
            solver_container[val_iZone][iMesh][TURB_SOL]->node[iPoint]->SetSolution(
                solver_container[val_iZone][iMesh][TURB_SOL]->node[iPoint]->GetSolution_time_n());
          }
        }
      }
      if (dual_time_1st) {
        ...
        for (iMesh = 0; iMesh <= config_container[val_iZone]->GetnMGLevels(); iMesh++) {
          for (iPoint = 0; iPoint < geometry_container[val_iZone][iMesh]->GetnPoint(); iPoint++) {
            solver_container[val_iZone][iMesh][FLOW_SOL]->node[iPoint]->Set_Solution_time_n(
                solver_container[val_iZone][iMesh][FLOW_SOL]->node[iPoint]->GetSolution_time_n1());
            if (turbulent) {
              solver_container[val_iZone][iMesh][TURB_SOL]->node[iPoint]->Set_Solution_time_n(
                  solver_container[val_iZone][iMesh][TURB_SOL]->node[iPoint]->GetSolution_time_n1());
            }
          }
        }
      }
      if (dual_time_2nd) {
        ...
        for (iMesh = 0; iMesh <= config_container[val_iZone]->GetnMGLevels(); iMesh++) {
          for (iPoint = 0; iPoint < geometry_container[val_iZone][iMesh]->GetnPoint(); iPoint++) {
            solver_container[val_iZone][iMesh][FLOW_SOL]->node[iPoint]->Set_Solution_time_n1(
                solver_container[val_iZone][iMesh][FLOW_SOL]->node[iPoint]->GetSolution_Old());
            if (turbulent) {
              solver_container[val_iZone][iMesh][TURB_SOL]->node[iPoint]->Set_Solution_time_n1(
                  solver_container[val_iZone][iMesh][TURB_SOL]->node[iPoint]->GetSolution_Old());
            }
          }
        }
      }
  ...
  if (ExtIter == 0 || dual_time) {
    for (iMesh = 0; iMesh <= config_container[val_iZone]->GetnMGLevels(); iMesh++) {
      for (iPoint = 0; iPoint < geometry_container[val_iZone][iMesh]->GetnPoint(); iPoint++) {
        solver_container[val_iZone][iMesh][ADJFLOW_SOL]->node[iPoint]->SetSolution_Direct(
            solver_container[val_iZone][iMesh][FLOW_SOL]->node[iPoint]->GetSolution());
      }
    }
    if (turbulent && !config_container[val_iZone]->GetFrozen_Visc_Disc()) {
      for (iPoint = 0; iPoint < geometry_container[val_iZone][MESH_0]->GetnPoint(); iPoint++) {
        solver_container[val_iZone][MESH_0][ADJTURB_SOL]->node[iPoint]->SetSolution_Direct(
            solver_container[val_iZone][MESH_0][TURB_SOL]->node[iPoint]->GetSolution());
      }
    }
  }

  solver_container[val_iZone][MESH_0][ADJFLOW_SOL]->Preprocessing(
      geometry_container[val_iZone][MESH_0], solver_container[val_iZone][MESH_0], config_container[val_iZone], MESH_0,
      0, RUNTIME_ADJFLOW_SYS, false);
  if (turbulent && !config_container[val_iZone]->GetFrozen_Visc_Disc()) {
    solver_container[val_iZone][MESH_0][ADJTURB_SOL]->Preprocessing(
        geometry_container[val_iZone][MESH_0], solver_container[val_iZone][MESH_0], config_container[val_iZone], MESH_0,
        0, RUNTIME_ADJTURB_SYS, false);
  }
}

void CDiscAdjFluidIteration::LoadUnsteady_Solution(CGeometry*** geometry_container, CSolver**** solver_container,
                                                   CConfig** config_container, unsigned short val_iZone,
                                                   int val_DirectIter) {
  unsigned short iMesh;
  ...
  if (val_DirectIter >= 0) {
    ...
    solver_container[val_iZone][MESH_0][FLOW_SOL]->LoadRestart(
        geometry_container[val_iZone], solver_container[val_iZone], config_container[val_iZone], val_DirectIter, true);
    if (turbulent) {
      solver_container[val_iZone][MESH_0][TURB_SOL]->LoadRestart(geometry_container[val_iZone],
                                                                 solver_container[val_iZone],
                                                                 config_container[val_iZone], val_DirectIter, false);
    }
  } else {
    ...
    for (iMesh = 0; iMesh <= config_container[val_iZone]->GetnMGLevels(); iMesh++) {
      ...
      solver_container[val_iZone][iMesh][FLOW_SOL]->Preprocessing(
          geometry_container[val_iZone][iMesh], solver_container[val_iZone][iMesh], config_container[val_iZone], iMesh,
      ...
      if (turbulent) {
        ...
        solver_container[val_iZone][iMesh][TURB_SOL]->Postprocessing(geometry_container[val_iZone][iMesh],
...
void CDiscAdjFluidIteration::Iterate(COutput* output, CIntegration*** integration_container,
                                     CGeometry*** geometry_container, CSolver**** solver_container,
                                     CNumerics***** numerics_container, CConfig** config_container,
  ...
  if ((Kind_Solver == DISC_ADJ_NAVIER_STOKES) || (Kind_Solver == DISC_ADJ_RANS) || (Kind_Solver == DISC_ADJ_EULER)) {
    ...
    integration_container[val_iZone][ADJFLOW_SOL]->Convergence_Monitoring(
        geometry_container[val_iZone][MESH_0], config_container[val_iZone], IntIter,
        log10(solver_container[val_iZone][MESH_0][ADJFLOW_SOL]->GetRes_RMS(0)), MESH_0);
  }
...
void CDiscAdjFluidIteration::InitializeAdjoint(CSolver**** solver_container, CGeometry*** geometry_container,
  ...
  solver_container[iZone][MESH_0][ADJFLOW_SOL]->SetAdj_ObjFunc(geometry_container[iZone][MESH_0],
...
void CDiscAdjFluidIteration::RegisterInput(CSolver**** solver_container, CGeometry*** geometry_container,
                                           CConfig** config_container, unsigned short iZone,
  ...
  if (kind_recording == FLOW_CONS_VARS || kind_recording == COMBINED) {
    ...
    if ((Kind_Solver == DISC_ADJ_NAVIER_STOKES) || (Kind_Solver == DISC_ADJ_RANS) || (Kind_Solver == DISC_ADJ_EULER)) {
      ...
      solver_container[iZone][MESH_0][ADJFLOW_SOL]->RegisterVariables(geometry_container[iZone][MESH_0],
                                                                      config_container[iZone]);
    }

    if ((Kind_Solver == DISC_ADJ_RANS) && !frozen_visc) {
      solver_container[iZone][MESH_0][ADJTURB_SOL]->RegisterSolution(geometry_container[iZone][MESH_0],
                                                                     config_container[iZone]);
    }
  }
...
void CDiscAdjFluidIteration::SetDependencies(CSolver**** solver_container, CGeometry*** geometry_container,
                                             CConfig** config_container, unsigned short iZone,
                                             unsigned short kind_recording) {
  unsigned short Kind_Solver = config_container[iZone]->GetKind_Solver();
  bool frozen_visc = config_container[iZone]->GetFrozen_Visc_Disc();
  if ((kind_recording == MESH_COORDS) || (kind_recording == NONE) || (kind_recording == GEOMETRY_CROSS_TERM) ||
      (kind_recording == ALL_VARIABLES)) {
  ...
  solver_container[iZone][MESH_0][FLOW_SOL]->Set_MPI_Solution(geometry_container[iZone][MESH_0],
                                                              config_container[iZone]);

  if ((Kind_Solver == DISC_ADJ_RANS) && !frozen_visc) {
    solver_container[iZone][MESH_0][FLOW_SOL]->Preprocessing(geometry_container[iZone][MESH_0],
                                                             solver_container[iZone][MESH_0], config_container[iZone],
                                                             MESH_0, NO_RK_ITER, RUNTIME_FLOW_SYS, true);
    solver_container[iZone][MESH_0][TURB_SOL]->Postprocessing(
        geometry_container[iZone][MESH_0], solver_container[iZone][MESH_0], config_container[iZone], MESH_0);
    solver_container[iZone][MESH_0][TURB_SOL]->Set_MPI_Solution(geometry_container[iZone][MESH_0],
                                                                config_container[iZone]);
  }
}

void CDiscAdjFluidIteration::RegisterOutput(CSolver**** solver_container, CGeometry*** geometry_container,
                                            CConfig** config_container, COutput* output, unsigned short iZone) {
  unsigned short Kind_Solver = config_container[iZone]->GetKind_Solver();
  bool frozen_visc = config_container[iZone]->GetFrozen_Visc_Disc();
  ...
  if ((Kind_Solver == DISC_ADJ_NAVIER_STOKES) || (Kind_Solver == DISC_ADJ_RANS) || (Kind_Solver == DISC_ADJ_EULER)) {
    ...
    solver_container[iZone][MESH_0][FLOW_SOL]->RegisterOutput(geometry_container[iZone][MESH_0],
                                                              config_container[iZone]);
  }
  if ((Kind_Solver == DISC_ADJ_RANS) && !frozen_visc) {
...
void CDiscAdjFluidIteration::InitializeAdjoint_CrossTerm(CSolver**** solver_container, CGeometry*** geometry_container,
                                                         CConfig** config_container, unsigned short iZone) {
  unsigned short Kind_Solver = config_container[iZone]->GetKind_Solver();
  bool frozen_visc = config_container[iZone]->GetFrozen_Visc_Disc();
...
void CDiscAdjFluidIteration::Update(COutput* output, CIntegration*** integration_container,
                                    CGeometry*** geometry_container, CSolver**** solver_container,
                                    CNumerics***** numerics_container, CConfig** config_container,
                                    CSurfaceMovement** surface_movement, CVolumetricMovement** grid_movement,
                                    CFreeFormDefBox*** FFDBox, unsigned short val_iZone) {
...
void CDiscAdjFluidIteration::Postprocess(COutput* output, CIntegration*** integration_container,
                                         CGeometry*** geometry_container, CSolver**** solver_container,
                                         CNumerics***** numerics_container, CConfig** config_container,
                                         CSurfaceMovement** surface_movement, CVolumetricMovement** grid_movement,
                                         CFreeFormDefBox*** FFDBox, unsigned short val_iZone) {}
...
void CDiscAdjFEAIteration::Preprocess(COutput* output, CIntegration*** integration_container,
                                      CGeometry*** geometry_container, CSolver**** solver_container,
                                      CNumerics***** numerics_container, CConfig** config_container,
                                      CSurfaceMovement** surface_movement, CVolumetricMovement** grid_movement,
                                      CFreeFormDefBox*** FFDBox, unsigned short val_iZone) {
  ...
  bool nonlinear_analysis =
      (config_container[val_iZone]->GetGeometricConditions() == LARGE_DEFORMATIONS);  // Nonlinear analysis.
  ...
  if (dynamic) {
    ...
    for (iPoint = 0; iPoint < geometry_container[val_iZone][MESH_0]->GetnPoint(); iPoint++) {
      solver_container[val_iZone][MESH_0][ADJFEA_SOL]->node[iPoint]->SetSolution_Direct(
          solver_container[val_iZone][MESH_0][FEA_SOL]->node[iPoint]->GetSolution());
    }

    for (iPoint = 0; iPoint < geometry_container[val_iZone][MESH_0]->GetnPoint(); iPoint++) {
      solver_container[val_iZone][MESH_0][ADJFEA_SOL]->node[iPoint]->SetSolution_Accel_Direct(
          solver_container[val_iZone][MESH_0][FEA_SOL]->node[iPoint]->GetSolution_Accel());
    }

    for (iPoint = 0; iPoint < geometry_container[val_iZone][MESH_0]->GetnPoint(); iPoint++) {
      solver_container[val_iZone][MESH_0][ADJFEA_SOL]->node[iPoint]->SetSolution_Vel_Direct(
          solver_container[val_iZone][MESH_0][FEA_SOL]->node[iPoint]->GetSolution_Vel());
    }
  ...
  solver_container[val_iZone][MESH_0][ADJFEA_SOL]->Preprocessing(
      geometry_container[val_iZone][MESH_0], solver_container[val_iZone][MESH_0], config_container[val_iZone], MESH_0,
      0, RUNTIME_ADJFEA_SYS, false);
    ...
    if (rank == MASTER_NODE && ((ExtIter == 0) || dynamic)) {
      if (nonlinear_analysis) {
        ...
        if (geometry_container[val_iZone][MESH_0]->GetnDim() == 2) {
          ...
          cout << "log10[RMS Ux]: " << log10(solver_container[val_iZone][MESH_0][FEA_SOL]->GetRes_RMS(0))
               << ", log10[RMS Uy]: " << log10(solver_container[val_iZone][MESH_0][FEA_SOL]->GetRes_RMS(1))
               << ", log10[RMS Uz]: " << log10(solver_container[val_iZone][MESH_0][FEA_SOL]->GetRes_RMS(2)) << "."
               << endl;
        }
      }
    }
...
void CDiscAdjFEAIteration::LoadDynamic_Solution(CGeometry*** geometry_container, CSolver**** solver_container,
                                                CConfig** config_container, unsigned short val_iZone,
                                                int val_DirectIter) {
  unsigned short iVar;
  unsigned long iPoint;
  ...
  if (val_DirectIter >= 0) {
    ...
    solver_container[val_iZone][MESH_0][FEA_SOL]->LoadRestart(geometry_container[val_iZone],
                                                              solver_container[val_iZone], config_container[val_iZone],
                                                              val_DirectIter, update_geo);
  } else {
    /*--- If there is no solution file we set the freestream condition ---*/
    if (rank == MASTER_NODE && val_iZone == ZONE_0)
...
void CDiscAdjFEAIteration::Iterate(COutput* output, CIntegration*** integration_container,
                                   CGeometry*** geometry_container, CSolver**** solver_container,
                                   CNumerics***** numerics_container, CConfig** config_container,
                                   CSurfaceMovement** surface_movement, CVolumetricMovement** volume_grid_movement,
                                   CFreeFormDefBox*** FFDBox, unsigned short val_iZone) {
  ...
  for (IntIter = 0; IntIter < nIntIter; IntIter++) {
    ...
    integration_container[val_iZone][ADJFEA_SOL]->Convergence_Monitoring(
        geometry_container[val_iZone][MESH_0], config_container[val_iZone], IntIter,
        log10(solver_container[val_iZone][MESH_0][ADJFLOW_SOL]->GetRes_RMS(0)), MESH_0);
  ...
  solver_container[val_iZone][MESH_0][ADJFEA_SOL]->SetSensitivity(geometry_container[val_iZone][MESH_0],
                                                                  config_container[val_iZone]);
...
void CDiscAdjFEAIteration::SetRecording(COutput* output, CIntegration*** integration_container,
                                        CGeometry*** geometry_container, CSolver**** solver_container,
                                        CNumerics***** numerics_container, CConfig** config_container,
                                        CSurfaceMovement** surface_movement, CVolumetricMovement** grid_movement,
                                        CFreeFormDefBox*** FFDBox, unsigned short val_iZone,
  ...
  if (CurrentRecording != kind_recording && (CurrentRecording != NONE)) {
    solver_container[val_iZone][MESH_0][ADJFEA_SOL]->SetRecording(geometry_container[val_iZone][MESH_0],
                                                                  config_container[val_iZone]);

    /*--- Clear indices of coupling variables ---*/

    SetDependencies(solver_container, geometry_container, numerics_container, config_container, val_iZone,
                    ALL_VARIABLES);
...
void CDiscAdjFEAIteration::RegisterInput(CSolver**** solver_container, CGeometry*** geometry_container,
                                         CConfig** config_container, unsigned short iZone,
                                         unsigned short kind_recording) {
  if (kind_recording == FEA_DISP_VARS) {
    ...
    solver_container[iZone][MESH_0][ADJFEA_SOL]->RegisterVariables(geometry_container[iZone][MESH_0],
                                                                   config_container[iZone]);
  }
...
void CDiscAdjFEAIteration::SetDependencies(CSolver**** solver_container, CGeometry*** geometry_container,
                                           CNumerics***** numerics_container, CConfig** config_container,
                                           unsigned short iZone, unsigned short kind_recording) {
  unsigned short iVar;
  ...
  for (iVar = 0; iVar < iMPROP; iVar++) {
    ...
    numerics_container[iZone][MESH_0][FEA_SOL][FEA_TERM]->SetMaterial_Properties(
        iVar, solver_container[iZone][MESH_0][ADJFEA_SOL]->GetVal_Young(iVar),
        solver_container[iZone][MESH_0][ADJFEA_SOL]->GetVal_Poisson(iVar));

    /*--- Add dependencies for Rho and Rho_DL ---*/

    numerics_container[iZone][MESH_0][FEA_SOL][FEA_TERM]->SetMaterial_Density(
        iVar, solver_container[iZone][MESH_0][ADJFEA_SOL]->GetVal_Rho(iVar),
    ...
    if (solver_container[iZone][MESH_0][FEA_SOL]->IsElementBased()) {
      ...
      numerics_container[iZone][MESH_0][FEA_SOL][MAT_NHCOMP]->SetMaterial_Properties(
          iVar, solver_container[iZone][MESH_0][ADJFEA_SOL]->GetVal_Young(iVar),
          solver_container[iZone][MESH_0][ADJFEA_SOL]->GetVal_Poisson(iVar));
      numerics_container[iZone][MESH_0][FEA_SOL][MAT_NHCOMP]->SetMaterial_Density(
          iVar, solver_container[iZone][MESH_0][ADJFEA_SOL]->GetVal_Rho(iVar),
          solver_container[iZone][MESH_0][ADJFEA_SOL]->GetVal_Rho_DL(iVar));

      /*--- Ideal DE ---*/
      numerics_container[iZone][MESH_0][FEA_SOL][MAT_IDEALDE]->SetMaterial_Properties(
          iVar, solver_container[iZone][MESH_0][ADJFEA_SOL]->GetVal_Young(iVar),
          solver_container[iZone][MESH_0][ADJFEA_SOL]->GetVal_Poisson(iVar));
      numerics_container[iZone][MESH_0][FEA_SOL][MAT_IDEALDE]->SetMaterial_Density(
          iVar, solver_container[iZone][MESH_0][ADJFEA_SOL]->GetVal_Rho(iVar),
          solver_container[iZone][MESH_0][ADJFEA_SOL]->GetVal_Rho_DL(iVar));

      /*--- Knowles ---*/
      numerics_container[iZone][MESH_0][FEA_SOL][MAT_KNOWLES]->SetMaterial_Properties(
          iVar, solver_container[iZone][MESH_0][ADJFEA_SOL]->GetVal_Young(iVar),
          solver_container[iZone][MESH_0][ADJFEA_SOL]->GetVal_Poisson(iVar));
      numerics_container[iZone][MESH_0][FEA_SOL][MAT_KNOWLES]->SetMaterial_Density(
          iVar, solver_container[iZone][MESH_0][ADJFEA_SOL]->GetVal_Rho(iVar),
          solver_container[iZone][MESH_0][ADJFEA_SOL]->GetVal_Rho_DL(iVar));
    }
  }
  ...
  if (config_container[iZone]->GetDE_Effects()) {
    ...
    for (unsigned short iEField = 0; iEField < nEField; iEField++) {
      numerics_container[iZone][MESH_0][FEA_SOL][FEA_TERM]->Set_ElectricField(
          iEField, solver_container[iZone][MESH_0][ADJFEA_SOL]->GetVal_EField(iEField));

      numerics_container[iZone][MESH_0][FEA_SOL][DE_TERM]->Set_ElectricField(
          iEField, solver_container[iZone][MESH_0][ADJFEA_SOL]->GetVal_EField(iEField));
    }
  }
  ...
  switch (config_container[iZone]->GetDV_FEA()) {
    ...
    case ELECTRIC_FIELD:
      ...
      for (unsigned short iDV = 0; iDV < nDV; iDV++) {
        numerics_container[iZone][MESH_0][FEA_SOL][FEA_TERM]->Set_DV_Val(
            iDV, solver_container[iZone][MESH_0][ADJFEA_SOL]->GetVal_DVFEA(iDV));

        if (config_container[iZone]->GetDE_Effects()) {
          numerics_container[iZone][MESH_0][FEA_SOL][DE_TERM]->Set_DV_Val(
              iDV, solver_container[iZone][MESH_0][ADJFEA_SOL]->GetVal_DVFEA(iDV));
        }
      }
      ...
      if (solver_container[iZone][MESH_0][FEA_SOL]->IsElementBased()) {
        for (unsigned short iDV = 0; iDV < nDV; iDV++) {
          numerics_container[iZone][MESH_0][FEA_SOL][MAT_NHCOMP]->Set_DV_Val(
              iDV, solver_container[iZone][MESH_0][ADJFEA_SOL]->GetVal_DVFEA(iDV));
          numerics_container[iZone][MESH_0][FEA_SOL][MAT_IDEALDE]->Set_DV_Val(
              iDV, solver_container[iZone][MESH_0][ADJFEA_SOL]->GetVal_DVFEA(iDV));
          numerics_container[iZone][MESH_0][FEA_SOL][MAT_KNOWLES]->Set_DV_Val(
              iDV, solver_container[iZone][MESH_0][ADJFEA_SOL]->GetVal_DVFEA(iDV));
        }
      }
...
void CDiscAdjFEAIteration::RegisterOutput(CSolver**** solver_container, CGeometry*** geometry_container,
                                          CConfig** config_container, unsigned short iZone) {
...
void CDiscAdjFEAIteration::InitializeAdjoint(CSolver**** solver_container, CGeometry*** geometry_container,
                                             CConfig** config_container, unsigned short iZone) {
  /*--- Initialize the adjoint of the objective function (typically with 1.0) ---*/

  solver_container[iZone][MESH_0][ADJFEA_SOL]->SetAdj_ObjFunc(geometry_container[iZone][MESH_0],
                                                              config_container[iZone]);
...
void CDiscAdjFEAIteration::InitializeAdjoint_CrossTerm(CSolver**** solver_container, CGeometry*** geometry_container,
                                                       CConfig** config_container, unsigned short iZone) {
  /*--- Initialize the adjoint of the objective function (typically with 1.0) ---*/
...
void CDiscAdjFEAIteration::Update(COutput* output, CIntegration*** integration_container,
                                  CGeometry*** geometry_container, CSolver**** solver_container,
                                  CNumerics***** numerics_container, CConfig** config_container,
                                  CSurfaceMovement** surface_movement, CVolumetricMovement** grid_movement,
                                  CFreeFormDefBox*** FFDBox, unsigned short val_iZone) {}
...
void CDiscAdjFEAIteration::Postprocess(COutput* output, CIntegration*** integration_container,
                                       CGeometry*** geometry_container, CSolver**** solver_container,
                                       CNumerics***** numerics_container, CConfig** config_container,
                                       CSurfaceMovement** surface_movement, CVolumetricMovement** grid_movement,
                                       CFreeFormDefBox*** FFDBox, unsigned short val_iZone) {
  ...
  for (iMarker = 0; iMarker < config_container[val_iZone]->GetnMarker_All(); iMarker++)
    switch (config_container[val_iZone]->GetMarker_All_KindBC(iMarker)) {
      case CLAMPED_BOUNDARY:
        solver_container[val_iZone][MESH_0][ADJFEA_SOL]->BC_Clamped_Post(
            geometry_container[val_iZone][MESH_0], solver_container[val_iZone][MESH_0],
            numerics_container[val_iZone][MESH_0][FEA_SOL][FEA_TERM], config_container[val_iZone], iMarker);
        break;
    }
}
...
void FEM_StructuralIteration(COutput* output, CIntegration*** integration_container, CGeometry*** geometry_container,
  ...
  bool nonlinear =
      (config_container[ZONE_0]->GetGeometricConditions() == LARGE_DEFORMATIONS);  // Geometrically non-linear problems
  ...
  if (!incremental_load) {
    ...
    //      solver_container[iZone][MESH_0][FEA_SOL]->SetInitialCondition(geometry_container[iZone],
    //      solver_container[iZone], config_container[iZone], ExtIter);
    ...
    for (iZone = 0; iZone < nZone; iZone++) {
      ...
      integration_container[iZone][FEA_SOL]->Structural_Iteration(
          geometry_container, solver_container, numerics_container, config_container, RUNTIME_FEA_SYS, IntIter, iZone);
    }
    ...
    if (nonlinear) {
      for (IntIter = 1; IntIter < config_container[ZONE_0]->GetDyn_nIntIter(); IntIter++) {
        for (iZone = 0; iZone < nZone; iZone++) {
          ...
          output->SetConvHistory_Body(NULL, geometry_container, solver_container, config_container,
                                      integration_container, true, 0.0, ZONE_0);

          config_container[iZone]->SetIntIter(IntIter);

          integration_container[iZone][FEA_SOL]->Structural_Iteration(geometry_container, solver_container,
                                                                      numerics_container, config_container,
                                                                      RUNTIME_FEA_SYS, IntIter, iZone);
        }
  ...
  /*--- The incremental load is only used in nonlinear cases ---*/
    ...
    for (iZone = 0; iZone < nZone; iZone++)
      solver_container[iZone][MESH_0][FEA_SOL]->SetInitialCondition(geometry_container[iZone], solver_container[iZone],
    ...
    /*--- If the criteria is not met, a whole set of subiterations for the different loads must be done ---*/
      ...
      for (iZone = 0; iZone < nZone; iZone++)
        solver_container[iZone][MESH_0][FEA_SOL]->ResetInitialCondition(
            geometry_container[iZone], solver_container[iZone], config_container[iZone], ExtIter);
  ...
  for (iZone = 0; iZone < nZone; iZone++)
    solver_container[iZone][MESH_0][FEA_SOL]->Compute_NodalStress(
        geometry_container[iZone][MESH_0], solver_container[iZone][MESH_0], numerics_container[iZone][MESH_0][FEA_SOL],
        config_container[iZone]);
