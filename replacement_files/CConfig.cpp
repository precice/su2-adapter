...
void CConfig::SetPostprocessing(SU2_COMPONENT val_software, unsigned short val_izone, unsigned short val_nDim) {
  ...
  if (Kind_FluidModel == SU2_NONEQ &&
    ...
    SU2_MPI::Error(
        "Only WILKE and SUTHERLAND transport models are stable for the NEMO solver using SU2TClib. Use Mutation++ "
        "instead.",
        CURRENT_FUNCTION);
  }
...
bool CConfig::GetVolumetric_Movement() const {
  ...
  if (GetSurface_Movement(AEROELASTIC) || GetSurface_Movement(AEROELASTIC_RIGID_MOTION) ||
      ...
      // preCICE: Ensure that volumetric_movement is activated so appropriate variables
      // (CVolumetricMovement.StiffMatrix) are instantiated
